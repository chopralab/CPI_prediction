...
if __name__ == "__main__":
    ...
    with open('../dataset/' + DATASET + '/original/'
              'smiles_sequence_test.txt', 'r') as f:
        test_list = f.read().strip().split('\n')

    """Exclude data contains "." in the smiles."""
    test_list = list(filter(lambda x:
                    '.' not in x.strip().split()[0], test_list))
    N = len(test_list)
    ...
    for no, test in enumerate(test_list):
        ...
        smiles, sequence = test.strip().split()
    ...
    print('The preprocess of test for ' + DATASET + ' dataset has finished!')
