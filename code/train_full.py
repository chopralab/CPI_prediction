...
class Tester(object):
    ...
    def result(self, epoch, time, loss,
               auc, precision, recall, file_name):
        with open(file_name, 'a') as f:
            result = map(str, [epoch, time, loss,
...
if __name__ == "__main__":
    ...
    file_result = '../output/full_model/' + setting + '.txt'
    with open(file_result, 'w') as f:
        f.write('Epoch\tTime(sec)\tLoss_train\t'
                'AUC\tPrecision\tRecall\n')

    file_model = '../output/full_model/' + setting
    ...
    for epoch in range(iteration):
        ...
        tester.result(epoch, time, loss,
                      auc, precision, recall, file_result)
        tester.save_model(model, file_model)

        print(epoch, time, loss, auc, precision, recall)
